...
describe('PROCESS_TASK_FORM_REGISTRY (M-1b)', () => {
  ...
  it('collects a single registration via provideProcessTaskForm', () => {
    TestBed.configureTestingModule({
      providers: [
        provideProcessTaskForm({ type: 'MT101_BUILD', component: TestMt101BuildFormComponent }),
      ],
    });

    const registrations = TestBed.inject(PROCESS_TASK_FORM_REGISTRY);
    expect(registrations).toHaveLength(1);
    expect(registrations[0].type).toBe('MT101_BUILD');
  ...
  it('aggregates multiple registrations via provideProcessTaskForms', () => {
    TestBed.configureTestingModule({
      providers: [
        ...provideProcessTaskForms(
          { type: 'MT101_BUILD', component: TestMt101BuildFormComponent, layout: 'workspace' },
    ...
    expect(registrations.map((r) => r.type).sort()).toEqual(['MT101_BUILD', 'MT101_VALIDATE']);

    const build = registrations.find((r) => r.type === 'MT101_BUILD')!;
  ...
  it('host can resolve registration by task type', () => {
    TestBed.configureTestingModule({
      providers: [
        ...provideProcessTaskForms(
          { type: 'MT101_BUILD', component: TestMt101BuildFormComponent },

...
| RF-006 | HU-01 | - | - | POST /api/source-definitions | source_definition (`s3` hecho; `gcs`/`azure-blob` WIP) | S3SourceProvider | - (integracion S3/MinIO pendiente) | S3 front+back (JVM); native/QA/gate pendiente | spec-tecnica.md (cloud) | source-s3-form (hecho) | - |
| RF-007 | HU-01 | - | - | POST /api/source-definitions | configuration_json (`authMode`: default/access-key; assume-role pendiente STS) | S3SourceProvider (DefaultCredentialsProvider/StaticCredentials) | - | S3 front+back (JVM); native/QA/gate pendiente | spec-tecnica.md (cloud) | source-s3-form (bloque auth) | - |
| RF-008 | HU-01 | - | - | POST /api/source-definitions/test | source_definition | SourcePayload (StreamSupplier) + S3 openFile (GetObject stream) | - | S3 hecho (streaming); native/QA pendiente | spec-tecnica.md (cloud) | - (insumo backend de FILE_READ) | - |

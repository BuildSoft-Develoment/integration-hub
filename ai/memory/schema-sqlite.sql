...
-- v12.122: Subagent review ledger (Capa 1 — execution discipline).
-- Persiste el ciclo de vida de cada T-NNN ejecutada por un agente y las reviews
-- 2-stage (spec-compliance + code-quality) por reviewers DIFERENTES del implementer
-- (anti-self-approval Principio 1). Ver ai/protocols/subagent-execution.md y code-review.md.
CREATE TABLE IF NOT EXISTS ai_task_runs (
  id INTEGER PRIMARY KEY,
  run_uuid TEXT NOT NULL UNIQUE,
  feature TEXT NOT NULL,
  task_id TEXT NOT NULL,                -- T-NNN
  agent TEXT NOT NULL,                  -- claude | codex | opencode | gemini | copilot | cursor | <other>
  status TEXT NOT NULL,                 -- in_progress | implementer_done | spec_review_passed | quality_review_passed | done_with_concerns | blocked | approved
  started_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  finished_at TEXT,
  worktree_path TEXT,
  UNIQUE (feature, task_id, run_uuid)
);
CREATE INDEX IF NOT EXISTS idx_ai_task_runs_feature ON ai_task_runs(feature, task_id);
CREATE INDEX IF NOT EXISTS idx_ai_task_runs_status ON ai_task_runs(status);

CREATE TABLE IF NOT EXISTS ai_task_reviews (
  id INTEGER PRIMARY KEY,
  run_uuid TEXT NOT NULL,
  stage TEXT NOT NULL,                  -- spec_compliance | code_quality
  reviewer_agent TEXT NOT NULL,         -- DEBE ser distinto del implementer (check:task-reviews)
  result TEXT NOT NULL,                 -- pass | fail | concerns
  reviewed_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  summary TEXT,
  FOREIGN KEY (run_uuid) REFERENCES ai_task_runs(run_uuid) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_ai_task_reviews_run ON ai_task_reviews(run_uuid);
CREATE INDEX IF NOT EXISTS idx_ai_task_reviews_stage ON ai_task_reviews(stage, result);

CREATE TABLE IF NOT EXISTS ai_task_review_findings (
  id INTEGER PRIMARY KEY,
  review_id INTEGER NOT NULL,
  severity TEXT NOT NULL,               -- blocker | major | minor
  area TEXT NOT NULL,                   -- trazabilidad | tdd | touch_policy | complexity | secrets | dead_code | lint | other
  message TEXT NOT NULL,
  file_path TEXT,
  line INTEGER,
  FOREIGN KEY (review_id) REFERENCES ai_task_reviews(id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_ai_task_findings_review ON ai_task_review_findings(review_id);
CREATE INDEX IF NOT EXISTS idx_ai_task_findings_severity ON ai_task_review_findings(severity);

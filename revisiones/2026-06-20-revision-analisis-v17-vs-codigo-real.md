...
| **B3'** | ✅ Núcleo hecho | **Paso 0** (verificación): el contador de `record_index` se cachea por `hash:location` (`DbWriteTaskProvider:305`), así que dos archivos byte-idénticos de orígenes distintos producen `(hash, record_index)` **solapados**; `staging_record` los conserva (sin único en `(hash, record_index)`) pero `mt101_fragment_record` (único `(set, hash, record_number)` + `on conflict do nothing`) **descartaba en silencio** el 2º. **Pasos 1-2** (V42): la identidad de `mt101_fragment_record`/`mt101_rebuild_selection` incluye `coalesce(staging_id,0)` → ya no se descarta; se capturan `source_task_definition_id` + `source_name` (de la tarea DB_WRITE) en el linaje. **Paso 4**: `Mt101MultiSourceLineageTest` prueba que dos archivos idénticos conservan ambos linajes con su origen. **Pendiente menor (Paso 3)**: exponer `source_name` en el lookup/UI (display-only; el dato ya está capturado). |

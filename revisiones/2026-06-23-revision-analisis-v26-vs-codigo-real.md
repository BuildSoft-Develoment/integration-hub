...
| 1 | El provider reconstruye el plan (no lo valida contra el persistido) | **REAL → CORREGIDO (2º pase)** | El claim ahora valida el **plan_hash completo** persistido: el provider recomputa `dispatch_plan_hash` (transport\|ruta\|destino\|correlación\|payload) y `markPayFragmentDispatching` exige que coincida con el del ledger; si difiere (transport/destino/correlación cambiados aunque payload+ruta sigan), el fragmento se **INVALIDA** y no se envía. "Plan aprobado = plan usado" demostrable. Tests `correctivePayInvalidatesFragmentWhenDispatchPlanChangedEvenIfPayloadAndRouteMatch` (+ los de payload/ruta) |
| 4 | RECONCILE aún usa config vigente (no congelada como STATUS) | **REAL → CORREGIDO (2º pase)** | **V57** `pay_reconcile_config_snapshot`: el PAY congela el perfil de MT101_RECONCILE (redactado), y la conciliación post-PAY y la diferida usan ese snapshot, no la config vigente. Simétrico al de STATUS. Test `resolveUncertainPayUsesTheReconcileProfileFrozenAtPayNotTheCurrentConfig` |
...
## Segundo pase (doble check) — #1 y #4 cerrados

### #1 — claim valida el plan completo persistido (plan aprobado = plan usado, demostrable)
`dispatchDestination`/`dispatchPlanHash` se movieron a `Mt101PayRouteResolver` (estáticos, compartidos por
servicio y provider). El provider, antes de despachar, **recomputa** `dispatch_plan_hash` y
`markPayFragmentDispatching` lo exige en su `WHERE` junto a `payload_hash` y `approved_routed_as`: si el
plan aprobado cambió (transport/destino/correlación, aunque payload+ruta sigan), el claim falla y el
fragmento se **INVALIDA** (no se envía). Ya no se infiere desde el hash de config: se valida el plan por
fragmento. Tests: plan distinto con mismo payload/ruta → INVALIDATED; payload/ruta cambiados → INVALIDATED.

### #4 — perfil de RECONCILE congelado
**V57** agrega `pay_reconcile_config_snapshot`. En `approveAndPayCorrective` se congela (redactado) la config
de MT101_RECONCILE además de la de STATUS; la conciliación inmediata (post-PAY) y la diferida (en
`resolveUncertainPay`) usan ese snapshot, no la config vigente. Test: el perfil de RECONCILE "deriva" tras
el PAY → la resolución concilia con el congelado.

## Documentado (infra, no bloqueante)

**HMAC/WORM/rol DB para la cadena.** La cadena hash (V56) es tamper-evident desde la app; firmarla con HMAC
(clave en Vault), exportar a WORM/Object Lock y un rol de BD exclusivo de inserción (revocar UPDATE/DELETE/
TRUNCATE al usuario aplicativo) son endurecimientos de infraestructura.

## Pendientes

Ninguno funcional. Los cuatro pendientes del v26 (StatusTransport SFTP, serialización de la cadena,
clasificación de excepción de transporte, plan persistido validado y RECONCILE congelado) quedaron cerrados
con prueba.

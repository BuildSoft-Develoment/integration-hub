...
// @trace spec 005-catalogo-conexiones RF-004, RF-005 (reingenieria: clase que implementa el/los RF en produccion)
...
public class ConnectionMetadataService {
    ...
    public List<ConnectionSchemaResponse> listSchemas(Long connectionDefinitionId) {
        ...
        return schemaNames.stream()
                ...
                .map(ConnectionSchemaResponse::new)
                .toList();
    }

    public List<ConnectionTableResponse> listTables(Long connectionDefinitionId, String schema, String query) {
        var definition = connectionDefinitionRepository.findRequired(connectionDefinitionId);
        var normalizedQuery = normalize(query);
        var tables = new ArrayList<ConnectionTableResponse>();
        withMetadata(definition, metadata -> {
            try (var rows = metadata.getTables(null, blankToNull(schema), "%", new String[]{"TABLE", "VIEW"})) {
                while (rows.next()) {
                    ...
                    tables.add(new ConnectionTableResponse(tableSchema, tableName, qualifiedName));
                }
            }
        });
        tables.sort(Comparator
                .comparing((ConnectionTableResponse item) -> item.schema() == null ? "" : item.schema(), String.CASE_INSENSITIVE_ORDER)
                .thenComparing(ConnectionTableResponse::name, String.CASE_INSENSITIVE_ORDER));
        return tables;
    }

    public List<ConnectionColumnResponse> listColumns(Long connectionDefinitionId, String schema, String table) {
        if (table == null || table.isBlank()) {
            throw new IllegalArgumentException("Table is required");
        }
        var definition = connectionDefinitionRepository.findRequired(connectionDefinitionId);
        var columns = new ArrayList<ConnectionColumnResponse>();
        withMetadata(definition, metadata -> {
            try (var rows = metadata.getColumns(null, blankToNull(schema), table, "%")) {
                while (rows.next()) {
                    columns.add(new ConnectionColumnResponse(
        ...
        columns.sort(Comparator.comparing(ConnectionColumnResponse::name, String.CASE_INSENSITIVE_ORDER));
        return columns;
    }

    public List<ConnectionRoutineResponse> listProcedures(Long connectionDefinitionId, String schema, String query) {
        var definition = connectionDefinitionRepository.findRequired(connectionDefinitionId);
        var normalizedQuery = normalize(query);
        var routines = new ArrayList<ConnectionRoutineResponse>();
        withMetadata(definition, metadata -> {
            try (var rows = metadata.getProcedures(null, blankToNull(schema), "%")) {
                while (rows.next()) {
                    ...
                    routines.add(new ConnectionRoutineResponse(routineSchema, routineName, qualifiedName, "PROCEDURE"));
                }
            }
        });
    ...
    public List<ConnectionRoutineParameterResponse> listProcedureParameters(Long connectionDefinitionId, String schema, String procedure) {
        if (procedure == null || procedure.isBlank()) {
            throw new IllegalArgumentException("Procedure is required");
        }
        var definition = connectionDefinitionRepository.findRequired(connectionDefinitionId);
        var parameters = new ArrayList<ConnectionRoutineParameterResponse>();
        withMetadata(definition, metadata -> {
            try (var rows = metadata.getProcedureColumns(null, blankToNull(schema), procedure, "%")) {
                while (rows.next()) {
                    ...
                    parameters.add(new ConnectionRoutineParameterResponse(
        ...
        parameters.sort(Comparator
                .comparing((ConnectionRoutineParameterResponse item) -> item.position() == null ? Integer.MAX_VALUE : item.position())
                .thenComparing(ConnectionRoutineParameterResponse::parameterName, String.CASE_INSENSITIVE_ORDER));
        return parameters;
    }

    public List<ConnectionRoutineResponse> listFunctions(Long connectionDefinitionId, String schema, String query) {
        var definition = connectionDefinitionRepository.findRequired(connectionDefinitionId);
        var normalizedQuery = normalize(query);
        var routines = new ArrayList<ConnectionRoutineResponse>();
        withMetadata(definition, metadata -> {
            try (var rows = metadata.getFunctions(null, blankToNull(schema), "%")) {
                while (rows.next()) {
                    ...
                    routines.add(new ConnectionRoutineResponse(routineSchema, routineName, qualifiedName, "FUNCTION"));
                }
            }
        });
        return sortRoutines(routines);
    ...
    public List<ConnectionRoutineParameterResponse> listFunctionParameters(Long connectionDefinitionId, String schema, String function) {
    ...
    private List<ConnectionRoutineResponse> sortRoutines(List<ConnectionRoutineResponse> routines) {
        routines.sort(Comparator
                .comparing((ConnectionRoutineResponse item) -> item.schema() == null ? "" : item.schema(), String.CASE_INSENSITIVE_ORDER)
                .thenComparing(ConnectionRoutineResponse::name, String.CASE_INSENSITIVE_ORDER));
        return routines;
    }

...
public class ProcessExecutionService {
        ...
        try {
            ...
            for (int index = 0; index < tasks.size(); index++) {
                ...
                if (shouldUseFileReadDbWritePipeline(taskPlan, nextTaskPlan)) {
                    ...
                    try {
                        var pipelineResult = processTaskRuntimeService.runFileReadToDbWrite(processExecutionId, taskPlan, nextTaskPlan, normalizedExecutionVariables, normalizedSelectedFiles);
                        ...
                        if (taskError instanceof ProcessTaskRuntimeService.FileReadDbWritePipelineException pipelineFailure) {
    ...
    private String buildReadDetails(String sourceName,
                                    ...
                                    List<ProcessTaskRuntimeService.FileReadSummary> fileSummaries) {
    ...
    private Map<String, Object> buildReadAuditPayload(ProcessExecutionStateService.TaskPlan taskPlan,
                                                      ReadResult readResult,
                                                      List<ProcessTaskRuntimeService.FileReadSummary> fileSummaries,
                                                      List<com.integrationhub.platform.spi.SelectedSourceFile> selectedFiles,
                                                      Map<String, String> executionVariables,
                                                      String triggerSource) {
    ...
    private String buildDbWriteDetails(ProcessExecutionStateService.TaskPlan dbWritePlan,
                                       ProcessTaskRuntimeService.FileReadDbWriteResult pipelineResult) {
    ...
    private String buildPipelineFailureDetails(String sourceName,
                                               ProcessTaskRuntimeService.FileReadDbWritePipelineException failure) {
        var lines = new StringBuilder()

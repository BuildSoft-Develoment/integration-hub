...
public class AsyncAvailabilityService {
    ...
    /**
     * Las precondiciones <b>del lado productor</b> (config/registro) para offloadar una tarea al backbone:
     * que el <b>relay</b> vaya a drenar el outbox ({@code tasks.dispatch.enabled}) hacia un <b>broker</b>
     * registrado. Lista vacía = despachable. Independiente de la liveness <b>transitoria</b>
     * ({@code dispatchLive}), que el outbox durable + relay aguantan.
     *
     * <p>La usa el despacho (§9) para <b>fallar ruidoso</b> en vez de suspender una tarea cuyo outbox nadie
     * drenaría (ni el recovery, que depende del mismo relay) → no-progreso silencioso. <b>Alcance productor,
     * a propósito</b>: NO incluye el <b>consumer</b> ({@code tasks-in}) — el patrón transactional-outbox
     * <i>desacopla</i> al productor de la entrega; el consumer vive aguas abajo del broker (otro nodo en un
     * despliegue distribuido) y su ausencia produce stalls que ya exponen el tile de salud async y los
     * endpoints DLQ/stalled, no el productor. Tampoco incluye {@code executionEnabled}: con async off toda
     * tarea corre síncrona por diseño (master switch, no un gap).</p>
     */
    public List<String> producerDispatchGaps() {
        var a = availability();
        return producerDispatchGaps(a.dispatchEnabled(), a.brokersRegistered());
    }

    /** Cómputo PURO de los gaps del productor (sin liveness ni consumer): testeable en aislamiento, como {@link #derive}. */
    static List<String> producerDispatchGaps(boolean dispatchEnabled, boolean brokersRegistered) {
        var gaps = new ArrayList<String>(2);
        if (!dispatchEnabled) {
            gaps.add("relay outbox→broker apagado (tasks.dispatch.enabled)");
        }
        if (!brokersRegistered) {
            gaps.add("sin broker registrado");
        }
        return gaps;
    }

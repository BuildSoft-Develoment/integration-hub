...
create table if not exists mt101_corrective_pay_plan (
...
create unique index if not exists ux_pay_plan_run_rev
    on mt101_corrective_pay_plan (rebuild_run_id, plan_revision);
...
create unique index if not exists ux_pay_plan_one_active
    on mt101_corrective_pay_plan (rebuild_run_id) where status = 'ACTIVE';
create unique index if not exists ux_pay_plan_one_draft
    on mt101_corrective_pay_plan (rebuild_run_id) where status = 'DRAFT';

create table if not exists mt101_corrective_pay_plan_fragment (
    id bigserial primary key,
    rebuild_run_id varchar(80) not null,
    plan_revision integer not null,
...
create unique index if not exists ux_pay_plan_frag_run_rev_ref
    on mt101_corrective_pay_plan_fragment (rebuild_run_id, plan_revision, corrective_senders_reference);

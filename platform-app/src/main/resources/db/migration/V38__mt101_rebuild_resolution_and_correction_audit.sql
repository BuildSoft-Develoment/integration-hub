...
create table if not exists mt101_staging_correction (
...
create index if not exists ix_mt101_staging_correction_row
    on mt101_staging_correction (fragment_set_id, source_file_hash, source_record_number, created_at);

create table if not exists mt101_reprocess_audit (
    id bigserial primary key,
...
create index if not exists ix_mt101_reprocess_audit_set
    on mt101_reprocess_audit (fragment_set_id, created_at);

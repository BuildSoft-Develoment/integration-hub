...
create table if not exists mt101_rebuild_run (
...
create unique index if not exists ux_mt101_rebuild_run_corrective
    on mt101_rebuild_run (corrective_set_id);

create index if not exists ix_mt101_rebuild_run_original_status
    on mt101_rebuild_run (original_fragment_set_id, status);

create table if not exists mt101_rebuild_selection (
...
create index if not exists ix_mt101_rebuild_selection_lookup
    on mt101_rebuild_selection (rebuild_run_id, record_index);

create index if not exists ix_mt101_rebuild_selection_original_ref
    on mt101_rebuild_selection (rebuild_run_id, original_senders_reference);

create table if not exists mt101_fragment_record (
    id bigserial primary key,
...
create index if not exists ix_mt101_fragment_record_source
    on mt101_fragment_record (source_file_hash, source_record_number);

create index if not exists ix_mt101_fragment_record_transaction
    on mt101_fragment_record (current_transaction_reference);

create index if not exists ix_mt101_fragment_record_rebuild
    on mt101_fragment_record (rebuild_run_id);

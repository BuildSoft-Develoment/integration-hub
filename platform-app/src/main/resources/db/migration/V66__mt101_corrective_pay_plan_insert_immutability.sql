...
create trigger trg_pay_plan_fragment_immutable
    before insert or update or delete on mt101_corrective_pay_plan_fragment
    for each row execute function mt101_pay_plan_fragment_immutable();

create or replace function mt101_pay_plan_insert_guard() returns trigger as $$
begin
...
drop trigger if exists trg_pay_plan_insert_guard on mt101_corrective_pay_plan;
create trigger trg_pay_plan_insert_guard
    before insert on mt101_corrective_pay_plan

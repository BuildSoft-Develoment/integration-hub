...
alter table payment_validation_rule
    alter column code type varchar(80);

create table if not exists mt101_build_fragment (
...
create unique index if not exists ux_mt101_build_fragment_set_index
    on mt101_build_fragment (fragment_set_id, fragment_index);

create unique index if not exists ux_mt101_build_fragment_set_ref
    on mt101_build_fragment (fragment_set_id, senders_reference);

create index if not exists ix_mt101_build_fragment_status
    on mt101_build_fragment (fragment_set_id, status);

create index if not exists ix_mt101_build_fragment_execution
    on mt101_build_fragment (process_execution_id, task_definition_id);

...
create index if not exists ix_mt101_rebuild_run_pay_status
    on mt101_rebuild_run (status, pay_status);

alter table mt101_failed_record
...
update mt101_failed_record failed
...
create index if not exists ix_mt101_failed_staging
    on mt101_failed_record (fragment_set_id, staging_id, status);

drop index if exists ux_mt101_failed_dedup;
...
create index if not exists ix_mt101_rebuild_selection_staging
    on mt101_rebuild_selection (rebuild_run_id, staging_id);

create index if not exists ix_mt101_fragment_record_rebuild_staging
    on mt101_fragment_record (rebuild_run_id, staging_id);

...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  /* v12.127: widget de las 3 capas + agent execution ledger */
  .capa-stack { display:grid; grid-template-columns:1fr; gap:8px; }
  @media(min-width:900px){ .capa-stack { grid-template-columns:repeat(3, 1fr); } }
  .capa { background:#F8FAFC; border:1px solid var(--line); border-left:4px solid #6B7280; border-radius:6px; padding:10px 12px; font-size:12px; }
  ...
  .agent-runs-list { display:flex; flex-direction:column; gap:8px; margin-bottom:8px; }
  .agent-run-row { background:#F8FAFC; border:1px solid var(--line); border-radius:4px; padding:8px 12px; font-size:12px; }
  .agent-run-head { display:flex; gap:8px; align-items:center; font-weight:600; color:var(--ink); margin-bottom:4px; }
  ...
  .pane-capa-banner.capa-1-banner { border-left-color:#7C3AED; background:#F5F3FF; }
  .pane-capa-banner.capa-2-banner { border-left-color:#1E40AF; background:#EFF6FF; }
...
<div class="wrap">
  ...
  <div class="pane" id="pane-gates">
    <div class="pane-capa-banner capa-2-banner"><strong>Capa 2 · Gates</strong> · datos en <code>ai_gate_runs</code>. <span class="muted">Anti-self-approval (Principio 1 de CONSTITUTION.md): agente solicita, humano aprueba. Verificado por <code>roadmap:audit</code> + <code>check:gate-status-format</code>.</span></div>
    <div id="tab-gates"></div>
  </div>
  <div class="pane" id="pane-decisions">
    <div class="pane-capa-banner capa-2-banner"><strong>Capa 2 · Decisiones</strong> · datos en <code>ai_decisions</code> + ADRs. <span class="muted">Capa 1: las decisiones de review (spec-compliance / code-quality) tambien quedan en <code>ai_task_reviews.summary</code> con findings detalladas en <code>ai_task_review_findings</code>.</span></div>
    <div id="tab-decisions"></div>
  </div>
  <div class="pane" id="pane-evidence">
    <div class="pane-capa-banner capa-2-banner"><strong>Capa 2 · Evidencia</strong> · datos en <code>ai_evidence_items</code>. <span class="muted">Complementada por Capa 1: <code>specs/&lt;slug&gt;/tdd-evidence.md</code> (RED+GREEN logs auditables, v12.120) + <code>.agent/runs/&lt;uuid&gt;/</code> (markdown de cada review 2-stage, v12.122).</span></div>
    <div id="tab-evidence"></div>
  </div>
  <div class="pane" id="pane-questions">
    <div class="pane-capa-banner capa-2-banner"><strong>Capa 2 · Preguntas abiertas</strong> · datos en <code>ai_open_questions</code>. <span class="muted">Capa 1 / fase 1: <code>npm run project:clarify -- --feature &lt;slug&gt;</code> detecta nuevas (placeholders, criterios sin Dado/Cuando/Entonces, actores sin rol).</span></div>
    <div id="tab-questions"></div>
  </div>
  <div class="pane" id="pane-docs">
    <div class="pane-capa-banner capa-2-banner"><strong>Capa 2 · Documentos</strong> · datos en <code>ai_documents</code> + <code>ai_document_chunks</code> (FTS5). <span class="muted">Indexa: <code>CONSTITUTION.md</code>, <code>AGENT_RUNTIME.md</code>, <code>AGENTS.md</code>, <code>docs/fase-*</code>, <code>ai/protocols/*</code>, <code>ai/skills/*.skill.md</code>, <code>plantillas/*</code> y <code>specs/*</code>.</span></div>
    <div id="tab-docs"></div>
  </div>
  ...
  <div class="pane" id="pane-actions">
    <div class="pane-capa-banner capa-1-banner"><strong>Capa 1 · Acciones</strong> · ejecuta validadores, sync de memoria, generadores y la disciplina de ejecucion (<code>agent:protocol</code> · <code>agent:start</code> · <code>agent:review</code> · <code>agent:finish</code>). <span class="muted">Cada accion deja registro en <code>ai_action_runs</code> y, para agent:*, en <code>ai_task_runs</code> + <code>ai_task_reviews</code>.</span></div>
  ...
  <div class="pane" id="pane-files">
    <div class="pane-capa-banner capa-2-banner"><strong>Capa 2 · Proyecto</strong> · visor read-only del filesystem completo. <span class="muted">Atajos de docs raiz: <code>CONSTITUTION.md</code> · <code>AGENTS.md</code> · <code>AGENT_RUNTIME.md</code> · <code>AI_CONTEXT.md</code> · <code>ROADMAP_STATE.json</code>. Capa 1 vive bajo <code>ai/protocols/</code> + <code>ai/skills/</code> + <code>scripts/agent-*.mjs</code>.</span></div>
    <div class="roadmap-toolbar">
  ...
  <div class="pane" id="pane-agents">
    <div class="pane-capa-banner capa-1-banner"><strong>Capa 1 · Multiagente</strong> · locks de feature (<code>roadmap:claim/release</code>) coexisten con <code>agent:start/finish</code> (locks de T-NNN via SQLite). <span class="muted">El widget "Active runs" abajo muestra qué agentes estan ejecutando qué tarea ahora mismo (datos en <code>ai_task_runs</code>).</span></div>
    <div class="roadmap-toolbar">
...
const EXEC_ACTIONS = [
  ...
  // ── AGENT EXECUTION (Capa 1, v12.128): los 4 agent:* + validators de execution discipline ────
  // Ver AGENT_RUNTIME.md + ai/protocols/. Estos comandos son la ejecucion disciplinada del agente.
  { id: "agent-protocol",     category: "agent",     label: "agent:protocol",             hint: "Selector heuristico de protocolo aplicable a una tarea (input: descripcion).",      scriptPath: "scripts/agent-protocol.mjs", arg: { name: "--task", required: true, hint: "descripcion breve de la tarea" } },
  { id: "agent-start",        category: "agent",     label: "agent:start (worktree+claim)", hint: "Inicio orquestado de T-NNN: lock + worktree + baseline + context pack + protocolo.", scriptPath: "scripts/agent-start.mjs", arg: { name: "--feature", required: true, hint: "NNN-slug (la tarea + agente se pasan en --task y --agent dentro)" }, danger: true },
  { id: "agent-review",       category: "agent",     label: "agent:review (2-stage)",     hint: "Spec-compliance + code-quality review (reviewer != implementer — Principio 1).",     scriptPath: "scripts/agent-review.mjs", arg: { name: "--feature", required: true, hint: "NNN-slug (pasa --task, --stage both, --reviewer en una sola llamada)" } },
  { id: "agent-finish",       category: "agent",     label: "agent:finish (verify+opciones)", hint: "Verifica + checks + opciones humanas PR/merge/keep/discard.",                  scriptPath: "scripts/agent-finish.mjs", arg: { name: "--feature", required: true, hint: "NNN-slug", regex: "^\\d{3,}-[a-z0-9-]+$" }, danger: true },
  // Validadores de Capa 1 (corren tambien en check:project, pero exponer atajos individuales).
  { id: "check-tasks-executable", category: "validador", label: "check:tasks-executable", hint: "Tabla strict 12 cols en spec-tareas: sin placeholders/paths inventados/comandos vacios.", argv: null, ciScript: "check-tasks-executable.mjs", acceptsDb: false },
  { id: "check-tdd-evidence",     category: "validador", label: "check:tdd-evidence",     hint: "Cada T tipo=impl en state=done debe tener RED+GREEN+Verified reales en tdd-evidence.md.", argv: null, ciScript: "check-tdd-evidence.mjs", acceptsDb: false },
  { id: "check-task-reviews",     category: "validador", label: "check:task-reviews",     hint: "Cada T en state=done tiene 2 reviews (spec+quality) en SQLite con reviewer != implementer.", argv: null, ciScript: "check-task-reviews.mjs", acceptsDb: true },
  { id: "check-protocols",        category: "validador", label: "check:protocols",        hint: "Los 6 protocolos ai/protocols/* tienen estructura canonica (Capa 1).", argv: null, ciScript: "check-protocols.mjs", acceptsDb: false },
  { id: "check-skills",           category: "validador", label: "check:skills",           hint: "7 skills foundacionales presentes + estructura canonica.", argv: null, ciScript: "check-skills.mjs", acceptsDb: false },
  // Diagnostico (v12.126).
  { id: "check-all-trace",        category: "reporte",   label: "check:all:trace (timing)", hint: "Bisecta hangs: corre cada validador con timing + timeout (default 120s).", scriptPath: "scripts/check-all-trace.mjs" },
  // Project commands (Capa 2 / fase 1).
  { id: "project-clarify",        category: "agent",     label: "project:clarify",        hint: "Preguntas de desambiguacion sobre spec-funcional ANTES de cerrar fase 1.", scriptPath: "scripts/project-clarify.mjs", arg: { name: "--feature", required: true, hint: "NNN-slug o --all" } },
  { id: "project-analyze",        category: "agent",     label: "project:analyze",        hint: "Coherencia cross-artefacto (spec/tecnica/api/traceability/tareas).", scriptPath: "scripts/project-analyze.mjs", arg: { name: "--feature", required: true, hint: "NNN-slug o --all" } },
  { id: "project-checklist",      category: "agent",     label: "project:checklist",      hint: "Checklist auto + humano por feature.", scriptPath: "scripts/project-checklist.mjs", arg: { name: "--feature", required: true, hint: "NNN-slug" } },
];
...
async function serveMemory(root, db, dbPath, port) {
  ...
  const liveHtml = memoryHtmlShell('window.__MEMORY__ = null; window.__MEMORY_MODE__ = "live";');
  const server = http.createServer((req, res) => {
    ...
    try {
      ...
      // v12.127: Agent execution ledger — refleja Capa 1 (execution discipline)
      // en el panel. Devuelve los runs mas recientes + reviews + findings counts.
      // Las 3 tablas las introdujo v12.122 (ai_task_runs, ai_task_reviews,
      // ai_task_review_findings).
      if (url.pathname === "/api/agent-runs") {
        try {
          const limit = Math.max(1, Math.min(50, Number(url.searchParams.get("limit") || 10)));
          // Verifica que las tablas existan (proyectos viejos pueden no tenerlas aun).
          const hasTable = (name) => !!db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name=?`).get(name);
          if (!hasTable("ai_task_runs")) {
            res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
            res.end(JSON.stringify({ runs: [], reviews: [], findings_by_run: {}, schema_missing: true }));
            return;
          }
          const runs = db.prepare(`SELECT id, run_uuid, feature, task_id, agent, status, started_at, finished_at, worktree_path FROM ai_task_runs ORDER BY started_at DESC LIMIT ?`).all(limit);
          const reviews = runs.length === 0 ? [] : db.prepare(`SELECT id, run_uuid, stage, reviewer_agent, result, reviewed_at, summary FROM ai_task_reviews WHERE run_uuid IN (${runs.map(() => "?").join(",")}) ORDER BY reviewed_at DESC`).all(...runs.map(r => r.run_uuid));
          const findings = reviews.length === 0 ? [] : db.prepare(`SELECT review_id, severity FROM ai_task_review_findings WHERE review_id IN (${reviews.map(() => "?").join(",")})`).all(...reviews.map(r => r.id));
          const findingsByReview = {};
          for (const f of findings) {
            findingsByReview[f.review_id] = findingsByReview[f.review_id] || { blocker: 0, major: 0, minor: 0 };
            findingsByReview[f.review_id][f.severity] = (findingsByReview[f.review_id][f.severity] || 0) + 1;
          }
          // Agrupa reviews por run.
          const reviewsByRun = {};
          for (const rv of reviews) {
            reviewsByRun[rv.run_uuid] = reviewsByRun[rv.run_uuid] || [];
            reviewsByRun[rv.run_uuid].push({ ...rv, findings_count: findingsByReview[rv.id] || { blocker: 0, major: 0, minor: 0 } });
          }
          res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
          res.end(JSON.stringify({ runs, reviewsByRun, total: runs.length, schema_missing: false }));
        } catch (e) {
          res.writeHead(500, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: String(e.message || e) }));
        }

...
> ## ⚠️ EL GATING DESCRITO NO EXISTE
>
> Este documento afirma que "el gating es automatico" mediante el job `contract-governance` del
> workflow `template.yml`, comparando contra la rama `main`. Verificado hoy: **no existe ese job,
> no existe `template.yml`, y no existe la rama `main`** (las ramas reales son `master` y las de
> trabajo).
>
> El unico chequeo real de contrato es `ci/scripts/check-api-vs-code.mjs`, que hasta hoy tampoco se
> ejecutaba porque el CI apuntaba a ramas inexistentes. Su primera medicion: **123 endpoints en
> codigo contra 40 en `openapi.yaml`** — 83 sin contrato. Ese es el punto de partida real.
>
> Ningun PR ha sido bloqueado nunca por romper un contrato.
...
- **Changelog del API**: `contracts/CHANGELOG.md **(no existe todavia)**` (separado del CHANGELOG del repo) registra cada cambio con fecha y categoria.
...
- [Versionado y commits (90.19)](90.19-versionado-commits.md)

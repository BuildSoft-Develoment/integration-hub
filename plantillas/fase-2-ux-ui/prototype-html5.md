...
- Forma soportada de crearlo: `npm run scaffold:prototype -- --feature <NNN-slug>` (default freeform v12.116+; el agente diseña desde los RF). Opt-in `--domain X` para copiar un golden como punto de partida (desaconsejado). El validador `check:prototype-location` bloquea (strict en `pre-flight-gate` de proyecto instanciado) si está fuera de lugar.

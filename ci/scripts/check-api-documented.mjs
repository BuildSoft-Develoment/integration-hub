 ...
 * (ej. `GET /api/expedientes`), verifica que exista doc canonica en algun
...
// v12.126: suprime ExperimentalWarning de node:sqlite (causa hangs en Windows con npm pipe).
...
// Normalizar endpoint: "GET /api/expedientes" -> { method: "GET", path: "/api/expedientes" }
...
function walkContracts(dir) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      openapiExists = true;
      const text = readFileSync(full, "utf8");
      // Parser ligero: buscar paths como "  /api/expedientes:" seguido (lineas
...
if (missing.length > 0) {
  ...
  console.error("Formato canonico: '## Endpoint' + linea 'METHOD /path' (ej. 'GET /api/expedientes').");
  exitCode = 1;
}
if (missingFromOpenapi.length > 0) {
  ...
  console.error("  paths:");
  console.error("    /api/expedientes:");
  console.error("      get:");
  console.error("        summary: Lista expedientes");
  exitCode = 1;
}

...
const SEPARATE_PIPELINE_VALIDATORS = new Set([
  ...
  // v12.129: check-test-bundle es opt-in (requiere --root <path-extraido> o TEST_BUNDLE=1).
  // Se invoca via `npm run check:test-bundle` por reviewers; no entra en check:project/template.
  "check-test-bundle.mjs",
  // v12.130: check-bundle-release tambien opt-in (requiere --zip <path>). Valida un ZIP
  // generado sin desempaquetarlo. Se invoca via `npm run check:bundle:release -- --zip ...`.
  "check-bundle-release.mjs",
  // v12.134: plugin validators opt-in (manifest = WARN default, no-project-data requiere --plugin-zip).
  "check-plugin-manifest.mjs",
  "check-plugin-no-project-data.mjs",
]);

...
export function summarizePrototypeStates(root) {
  ...
  // v12.117: estructura granular para JSON. Antes solo emitiamos `phase2to3Ready: boolean`
  // que un agente IA leyendo el JSON podia malinterpretar como "todo el proyecto puede
  // pasar fase 2 -> 3" cuando en realidad solo aplica a las features con prototipo
  // human-approved. Ahora la verdad esta DESGLOSADA:
  //   project_ready   - true sii TODAS las features visuales tienen prototipo human-approved
  //                     (o no hay features visuales). Hoy: con cualquier feature sin proto
  //                     o no human-approved -> false.
  //   ready_features  - slugs cuyo prototipo SI esta human-approved.
  //   blocked_features- slugs con prototipo NO aprobado o sin prototipo.
  // El boolean legacy `phase2to3Ready` se conserva como atajo en el JSON, pero su semantica
  // hoy es "hay AL MENOS 1 feature con prototipo y todas las que tienen prototipo estan
  // human-approved" (no es "todo el proyecto"). El front embebido debe leer `phase2to3` para
  // mensajes sin ambiguedad.
  const ready = withProto.filter((f) => f.state === "human-approved").map((f) => f.slug);
  const blocked = features
    .filter((f) => f.state !== "human-approved")
    .map((f) => f.slug);
  const projectReady = features.length > 0 && blocked.length === 0;
  ...
  return {
    ...
    counts,
    features,
    // Avance fase 2 -> 3 (atajo legacy). Su semantica acotada: existe AL MENOS 1
    // prototipo y todos los existentes estan aprobados — pero NO garantiza que el
    // proyecto entero avanza. Para evitar malinterpretacion, usar `phase2to3`.
    phase2to3Ready: withProto.length > 0 && blockers.length === 0,
    phase2to3Blockers: blockers,
    // Estructura granular (v12.117).
    phase2to3: {

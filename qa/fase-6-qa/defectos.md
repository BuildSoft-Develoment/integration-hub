...
| QA-008-002 | Brecha de cobertura | Media | Parcial | REST 4xx/5xx, retries, idempotencia y SFTP con contenedor real quedaron cubiertos. Falta homologar contra gateway bancario real y rechazos semanticos certificados. | `RestPaymentTransportTest`, `SftpPaymentTransportTest` |
...
| QA-008-005 | Observacion tecnica | Baja | Abierto | Nx marco `web:test` como flaky aunque los 50 archivos y 167 tests pasaron; conviene aislar la causa antes del gate final. | `cmd.exe /c "cd frontend && npx nx test web --skip-nx-cache"` |

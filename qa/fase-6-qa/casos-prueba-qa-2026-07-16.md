...
| **Amazon S3** | Region=`us-east-1` · Bucket=`ih-source-inbox` · Endpoint=`http://minio:9000` · Auth mode=`access-key` **(obligatorio: sin acceso por llave cae a la cadena AWS -> "Unable to load credentials")** · Access key ID=`minioadmin` · Secret access key=`minioadmin` · Path-style access=`ON` (obligatorio MinIO) · Selection mode=`all` |
...
| **TXT** (ancho fijo) | **Modo=`Ancho fijo` (`fixed-length`) — obligatorio** (por defecto es Delimitado y los campos salen `null`) · Campos con posiciones: `reference` Start=`1` End=`16`; `amount` Start=`17` End=`32` Type=`NUMBER` |
...
| CSRC-03 | Manual-QA | Crear fuente Amazon S3 / MinIO | Login admin (P); minio arriba (bucket ih-source-inbox) | 1) Nueva fuente  2) Nombre = Fuente S3 QA ; Tipo = Amazon S3  3) Region = us-east-1 ; Bucket = ih-source-inbox ; Endpoint = http://minio:9000  4) Auth mode = access-key -> Access key ID = minioadmin ; Secret access key = minioadmin  5) Path-style access = ON ; Selection mode = all ; Media type = text/plain  6) Probar  7) Guardar | Probar OK contra MinIO; se guarda como Activa. Nota (verificado en int): MinIO exige Path-style access = ON; y Auth mode = access-key es OBLIGATORIO (con Access key ID + Secret access key). Sin authMode:access-key el backend cae a la cadena de credenciales AWS por defecto y falla con 'Unable to load credentials' | Alta | Funcional |
...
| CRDR-05 | Manual-QA | Crear reader TXT de ancho fijo | Login admin (P) | 1) Nuevo reader  2) Nombre = Reader TXT QA ; Tipo = TXT  3) Modo = Ancho fijo (fixed-length) -- OBLIGATORIO: el modo por defecto es Delimitado y con el los campos salen null  4) Agregar campos con Start/End/Size: reference Start = 1 End = 16 ; amount Start = 17 End = 32 Type = NUMBER  5) Guardar | Se guarda la definicion de posiciones fijas por campo. Nota (verificado en int): sin Modo = Ancho fijo (mode:fixed-length) el reader parsea toda la linea como un solo valor y los campos por Start/End salen null | Media | Funcional |
...
| SRC-06 | Tecnico/Dev | Lectura por S3/MinIO | bucket ih-source-inbox | 1) Subir un objeto al bucket  2) Ejecutar la fuente S3 (Auth mode = access-key) | El objeto se lee. Requiere authMode:access-key (ver CSRC-03); sin el cae a la cadena AWS y da 'Unable to load credentials' | Media | Funcional |
...
| SRC-08 | Tecnico/Dev | Formato TXT/FIN | - | 1) Ingresar un archivo TXT/FIN con un reader TXT en Modo = Ancho fijo (fixed-length) | Se parsea a staging. El reader TXT requiere mode:fixed-length (ver CRDR-05); con el modo por defecto (delimited) los campos por Start/End salen null | Media | Funcional |

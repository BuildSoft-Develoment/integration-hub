...
| Pruebas frontend | PASS con advertencia | `cmd.exe /c "cd frontend && npx nx test web --skip-nx-cache"`: 50 archivos y 168 tests pasaron. Nx reporto `web:test` como flaky aunque la ejecucion termino exitosa. |
...
| Frontend | `cmd.exe /c "cd frontend && npx nx test web --skip-nx-cache"` | PASS | 50 archivos de prueba y 168 tests pasaron. Queda advertencia de flakiness reportada por Nx. |

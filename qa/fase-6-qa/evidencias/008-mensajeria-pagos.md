...
| `MT101_PAY` | `MT101_PAY` | VERDE tecnico | REST, SFTP, retries, idempotencia, reproceso por fragmento y PAY correctivo con hash/lease/estado parcial. |
...
| Reproceso | Cubierto tecnico: reproceso por `fragmentSource.statuses=["REJECTED"]` sin regenerar el lote; correccion por `stagingId` obligatorio, sin fallback por hash/fila; cuarentena exige linaje exacto; rebuild masivo usa tabla de seleccion; lifecycle soporta `PARTIALLY_FAILED`, `PARTIALLY_SENT`, `INVALIDATED` y `UNCERTAIN` sin marcar todo el lote como fallido/enviado. Pendiente: escenario web/operativo con seleccion de lote o fragmento desde UI. |

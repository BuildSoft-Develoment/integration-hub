...
| `MT101_PAY` | `MT101_PAY` | VERDE tecnico | REST, SFTP, retries, idempotencia, reproceso por fragmento y PAY correctivo con hash/lease/estado parcial, intencion `PREPARED`, marca `DISPATCHING` previa al transporte y resolucion de `UNCERTAIN` sin reenvio. |
| `MT101_ROUTE` | `MT101_ROUTE` | VERDE tecnico | Routing outbound/inbound probado y fuente persistida `{fragmentSetId}` cubierta con persistencia `routed_as` por fragmento. |
...
| `MT101_ROUTE` | VERDE tecnico | Clasifica mensajes hacia rutas de pago/revision por reglas y persiste ruta en fragmentos masivos/correctivos. | Probado outbound, inbound y fuente persistida `{fragmentSetId}` con `routed_as`. | Routing negativo por reglas conflictivas, prioridad y ruta por defecto auditada. |

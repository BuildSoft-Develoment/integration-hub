...
update vertical_mt101.mt101_archive archive
set sender_lt = envelope.sender_lt
from vertical_mt101.swift_message_envelope envelope
where archive.envelope_id = envelope.id
  and archive.sender_lt is null;

drop index if exists vertical_mt101.ux_mt101_archive_idempotency;

create unique index if not exists ux_mt101_archive_operational_idempotency
    on vertical_mt101.mt101_archive (
        coalesce(sender_lt, ''),
        senders_reference,
...
create index if not exists ix_mt101_validation_fragment
    on vertical_mt101.mt101_validation_issue (fragment_set_id, senders_reference);

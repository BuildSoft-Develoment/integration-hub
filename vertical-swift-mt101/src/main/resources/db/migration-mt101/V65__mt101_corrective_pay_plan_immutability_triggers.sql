...
create trigger trg_pay_plan_fragment_immutable
    before update or delete on vertical_mt101.mt101_corrective_pay_plan_fragment
    for each row execute function vertical_mt101.mt101_pay_plan_fragment_immutable();

create or replace function vertical_mt101.mt101_pay_plan_status_guard() returns trigger as $$
begin
...
drop trigger if exists trg_pay_plan_status_guard on vertical_mt101.mt101_corrective_pay_plan;
create trigger trg_pay_plan_status_guard
    before update on vertical_mt101.mt101_corrective_pay_plan
    for each row execute function vertical_mt101.mt101_pay_plan_status_guard();

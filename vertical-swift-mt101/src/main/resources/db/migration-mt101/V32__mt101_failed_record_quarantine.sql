...
create table if not exists vertical_mt101.mt101_failed_record (
...
create index if not exists ix_mt101_failed_set
    on vertical_mt101.mt101_failed_record (fragment_set_id, status);

create index if not exists ix_mt101_failed_source_row
    on vertical_mt101.mt101_failed_record (source_file_hash, source_record_number);
